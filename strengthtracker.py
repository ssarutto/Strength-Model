"""
StrengthTracker — Workout Tracker, Strength Predictor & Programming Assistant
================================================================================
A Streamlit application that uses a domain-adapted Kalman Filter to estimate
true latent strength from noisy workout observations, modeling fatigue
accumulation, stress, and progressive adaptation.

Deploy to Streamlit Cloud (free) for instant mobile access:
  https://streamlit.io/cloud

Friends can open the link on their phones, log in, and "Add to Home Screen"
...
# =============================================================================
# CUSTOM CSS — Mobile-First, Clean Aesthetic
# =============================================================================
st.markdown("""
<style>
    .stButton>button {
...
class Database:
    ...
    def add_lift(self, name: str) -> int:
        with self._get_conn() as conn:
            try:
                c = conn.execute("INSERT INTO lifts (name) VALUES (?)", (name,))
                return c.lastrowid
            except sqlite3.IntegrityError:
                row = conn.execute("SELECT id FROM lifts WHERE name = ?", (name,)).fetchone()
                return row[0]

    def get_lifts(self) -> List[Dict]:
        with self._get_conn() as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute("SELECT * FROM lifts ORDER BY name").fetchall()
            return [dict(r) for r in rows]

    def add_workout(self, lift_id: int, workout_date: str, notes: str = "") -> int:
        with self._get_conn() as conn:
            c = conn.execute(
                "INSERT INTO workouts (lift_id, date, notes) VALUES (?, ?, ?)",
                (lift_id, workout_date, notes)
            )
            return c.lastrowid

    def add_set(self, workout_id: int, set_number: int, weight: float, reps: int, rir: int):
        with self._get_conn() as conn:
            conn.execute(
                "INSERT INTO sets (workout_id, set_number, weight, reps, rir) VALUES (?, ?, ?, ?, ?)",
                (workout_id, set_number, weight, reps, rir)
            )

    def get_workouts_with_sets(self, lift_id: Optional[int] = None) -> pd.DataFrame:
        with self._get_conn() as conn:
            query = """
                SELECT 
                    w.id as workout_id,
    ...
    def get_workouts_summary(self, lift_id: Optional[int] = None) -> pd.DataFrame:
        with self._get_conn() as conn:
            query = """
                SELECT 
                    w.id,
    ...
    def save_params(self, lift_id: int, params: dict):
        with self._get_conn() as conn:
            conn.execute(
                """INSERT INTO model_params (lift_id, params) VALUES (?, ?)
                   ON CONFLICT(lift_id) DO UPDATE SET params=excluded.params, updated_at=CURRENT_TIMESTAMP""",
                (lift_id, json.dumps(params))
            )

    def load_params(self, lift_id: int) -> Optional[dict]:
        with self._get_conn() as conn:
            row = conn.execute("SELECT params FROM model_params WHERE lift_id = ?", (lift_id,)).fetchone()
...
# =============================================================================
# KALMAN FILTER STRENGTH MODEL
# =============================================================================
class StrengthModel:
    """
    Domain-adapted Kalman Filter for strength tracking.

    State: true latent strength (scalar)
    Observation: estimated 1RM from best set, degraded by fatigue

    Key design choices from domain critique:
    - Smoothed strength S_{t-1} used for prediction (not raw or rolling)
    - Residuals normalized by strength to fix scale mismatch
    - Fatigue scaled by lambda_f < 1 (fatigue does not subtract 1:1)
    - Adaptive measurement noise: big residual -> inflate R
    - RIR penalty: multiplier = 1 + 0.3*(3 - RIR)
    """
...
def run_model_for_lift(db: Database, lift_id: int, custom_params: Optional[dict] = None) -> Tuple[Optional[StrengthModel], pd.DataFrame]:
    df = db.get_workouts_with_sets(lift_id)
    if df.empty:
        return None, df

    saved = db.load_params(lift_id)
    ...
    sets = [WorkoutSet(r['weight'], r['reps'], r['rir'], r['set_number']) 
            for _, r in first_workout.iterrows()]
...
def generate_demo_data(db: Database):
    lift_id = db.add_lift("Bench Press")
    ...
    for week in range(12):
        for session_in_week in range(3):
            session_idx += 1
            d = start_date + timedelta(days=week * 7 + session_in_week * 2)

            true_strength += np.random.normal(0.4, 0.2)
            fatigue *= 0.65

            if session_in_week == 0:
                sets_data = [(275, 3, 2), (275, 3, 2), (275, 3, 3)]
                stress = 12.0
            elif session_in_week == 1:
                sets_data = [(255, 5, 3), (255, 5, 3), (255, 5, 3), (255, 5, 4)]
                stress = 14.0
            else:
                sets_data = [(265, 4, 2), (265, 4, 2), (265, 4, 3)]
                stress = 10.0

            fatigue += stress * 0.25

            performance_factor = 1 - 0.6 * (fatigue / 20)
            noise = np.random.normal(0, 3.5)

            wid = db.add_workout(lift_id, d.strftime('%Y-%m-%d'), f"Week {week+1} Session {session_in_week+1}")
            for i, (w, r, rir) in enumerate(sets_data):
                adjusted_weight = w * performance_factor + noise * 0.3
                db.add_set(wid, i+1, round(adjusted_weight, 1), r, rir)

    lift_id2 = db.add_lift("Squat")
    true_strength2 = 405.0
    fatigue2 = 0.0
    for week in range(8):
        for session_in_week in range(2):
            d = start_date + timedelta(days=week * 7 + session_in_week * 3)
            true_strength2 += np.random.normal(0.5, 0.3)
            fatigue2 *= 0.6

            if session_in_week == 0:
                sets_data = [(365, 3, 2), (365, 3, 2), (365, 3, 3)]
                stress = 15.0
            else:
                sets_data = [(335, 5, 3), (335, 5, 3), (335, 5, 4)]
                stress = 12.0

            fatigue2 += stress * 0.3
            performance_factor = 1 - 0.6 * (fatigue2 / 25)
            noise = np.random.normal(0, 5.0)

            wid = db.add_workout(lift_id2, d.strftime('%Y-%m-%d'), f"Week {week+1} Session {session_in_week+1}")
            for i, (w, r, rir) in enumerate(sets_data):
                adjusted_weight = w * performance_factor + noise * 0.3
                db.add_set(wid, i+1, round(adjusted_weight, 1), r, rir)
...
def create_dashboard_plot(model: StrengthModel, results_df: pd.DataFrame, lift_name: str, projection_weeks: int = 8):
    ...
    fig.add_trace(go.Scatter(
        ...
        fillcolor='rgba(59, 130, 246, 0.15)', name='95% CI',
        hoverinfo='skip'
    ), row=1, col=1)
...
# EXPORT TO EXCEL
# =============================================================================
def export_to_excel(db: Database, lift_id: Optional[int] = None) -> bytes:
    output = io.BytesIO()

    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        raw = db.get_workouts_with_sets(lift_id)
        if not raw.empty:
        ...
        all_results = []
        for lift in lifts:
            model, results = run_model_for_lift(db, lift['id'])
        ...
        if all_results:
            ...
            combined.to_excel(writer, sheet_name='Model Results', index=False)

        summary = db.get_workouts_summary(lift_id)
...
def page_dashboard(db: Database):
    ...
    model, results = run_model_for_lift(db, lift_id)
...
def page_log_workout(db: Database):
    ...
    if st.button("💾 Save Workout", type="primary", use_container_width=True):
        ...
        lid = db.add_lift(lift_name)
        wid = db.add_workout(lid, workout_date.strftime('%Y-%m-%d'), notes)

        for i, (w, r, rir_val) in enumerate(set_data):
            db.add_set(wid, i+1, w, r, rir_val)
...
def page_export(db: Database):
    ...
    if st.button("📥 Download Excel", type="primary"):
        excel_data = export_to_excel(db, lift_id)
        st.download_button(
            label="Click to Download",
            data=excel_data,
            file_name=f"strengthtracker_export_{datetime.now().strftime('%Y%m%d')}.xlsx",
...
def page_settings(db: Database):
    st.title("⚙️ Settings")

    st.markdown("### Model Parameters")
    st.caption("These parameters control how the Kalman Filter estimates your strength. Adjust if your predictions feel off.")
    ...
    with col2:
        gamma = st.number_input("Residual Correction (gamma)", value=8.0, step=1.0, format="%.1f",
                                help="How much underperformance (negative residual) boosts fatigue estimate")
        lambda_f = st.number_input("Fatigue Scale (lambda_f)", value=0.60, step=0.05, format="%.2f",
                                   help="How much fatigue subtracts from performance (<1 means not 1:1)")
    ...
    if lifts:
        ...
        if st.button("💾 Save Parameters", type="primary"):
            db.save_params(lift_id, params)
            st.success(f"Parameters saved for **{selected}**!")

    st.markdown("---")
    st.markdown("### Demo Data")
    if st.button("🎲 Generate Demo Data", help="Creates 12 weeks of Bench Press and 8 weeks of Squat data"):
